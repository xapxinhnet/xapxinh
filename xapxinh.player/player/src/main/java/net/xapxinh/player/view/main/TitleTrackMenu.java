...
import net.xapxinh.player.view.action.mediaplayer.TitleAction;
...
final class TitleTrackMenu extends TrackMenu {
    ...
    protected Action createAction(TrackDescription trackDescription) {
        return new TitleAction(trackDescription.description(), application().mediaPlayerPanel().getMediaPlayer(), trackDescription.id());
    }

    @Override
    protected List<TrackDescription> onGetTrackDescriptions() {
        return application().mediaPlayerPanel().getMediaPlayer().getTitleDescriptions();
    }

    @Override
    protected int onGetSelectedTrack() {
        return application().mediaPlayerPanel().getMediaPlayer().getTitle();
    }
}
