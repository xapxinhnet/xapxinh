...
import net.xapxinh.player.view.action.mediaplayer.VideoTrackAction;
...
final class VideoTrackMenu extends TrackMenu {
    ...
    protected Action createAction(TrackDescription trackDescription) {
        return new VideoTrackAction(trackDescription.description(), application().mediaPlayerPanel().getMediaPlayer(), trackDescription.id());
    }

    @Override
    protected List<TrackDescription> onGetTrackDescriptions() {
        return application().mediaPlayerPanel().getMediaPlayer().getVideoDescriptions();
    }

    @Override
    protected int onGetSelectedTrack() {
        return application().mediaPlayerPanel().getMediaPlayer().getVideoTrack();
    }
}
