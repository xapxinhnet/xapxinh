...
import net.xapxinh.player.view.action.mediaplayer.AudioTrackAction;
...
final class AudioTrackMenu extends TrackMenu {
    ...
    protected Action createAction(TrackDescription trackDescription) {
        return new AudioTrackAction(trackDescription.description(), application().mediaPlayerPanel().getMediaPlayer(), trackDescription.id());
    }

    @Override
    protected List<TrackDescription> onGetTrackDescriptions() {
        return application().mediaPlayerPanel().getMediaPlayer().getAudioDescriptions();
    }

    @Override
    protected int onGetSelectedTrack() {
        return application().mediaPlayerPanel().getMediaPlayer().getAudioTrack();
    }
}
